...
class CenterlineAV2Dataset(BaseDataset):
    r"""NuScenes Dataset.
    ...
    def __init__(self,
                 ...
                 backend_args = None, front_camera_only=False,
                 **kwargs
                 ):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for raw_data_info in raw_data_list:
            ...
            data_info,num_points = self.parse_data_info(raw_data_info)
            if isinstance(data_info, dict):
                # For image tasks, `data_info` should information if single
                # image, such as dict(img_path='xxx', width=360, ...)
                data_list.append((num_points,data_info))
    ...
    def parse_data_info(self, info: dict) -> dict:
        ...
        if self.modality['use_camera']:
            ...
            extrinsics = []
            # img_timestamp = []
            for cam_type in ["ring_front_center",  "ring_front_right","ring_front_left","ring_rear_right","ring_rear_left", "ring_side_right","ring_side_left"]:
                cam_info=info['cams'][cam_type]
            # for cam_type, cam_info in info['cams'].items():
                if (not self.front_camera_only) or cam_type =="ring_front_center":
                    ...
                    extrinsics.append(cam_info['extrinsics'])  ###The extrinsics mean the tranformation from lidar to camera. If anyone want to use the extrinsics as sensor to lidar, please use np.linalg.inv(lidar2cam_rt.T) and modify the ResizeCropFlipImage and LoadMultiViewImageFromMultiSweepsFiles.
        ...
        if 'center_lines' in info.keys():
            ...
            num_points=sum([len(i) for i in info['center_lines']["centerlines"]])
        return input_dict,num_points