...
def main():
    ...
    for idx in selected:
        ...
        raw_vis = add_text(feature_to_rgb(raw_bev[0]), 'Raw BEV (LSS)')
        lpim_vis = add_text(feature_to_rgb(lpim_bev[0]), 'LPIM Injected BEV')

        panels = pad_to_same([raw_vis, lpim_vis])
        tile_h, tile_w = panels[0].shape[:2]
        canvas = np.zeros((tile_h * 1, tile_w * 2, 3), dtype=np.uint8)
        canvas[:, :tile_w] = panels[0]
        canvas[:, tile_w:tile_w * 2] = panels[1]
