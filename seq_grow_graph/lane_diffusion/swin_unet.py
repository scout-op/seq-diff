...
class SwinTransformerUNet(nn.Module):
    ...
    def forward(self, x, x_cond, t):
        ...
        # Need to flatten spatial dims for linear layer
        x = x.flatten(2).transpose(1, 2)  # [B, N, embed_dim]
        x_cond = x_cond.flatten(2).transpose(1, 2)  # [B, N, embed_dim]
        
        x = torch.cat([x, x_cond], dim=-1)  # [B, N, embed_dim * 2]
        x = self.fusion_linear(x)  # [B, N, embed_dim]
        
        # Reshape back for patch embed (which expects B, C, H, W)
