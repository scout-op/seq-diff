...
def test_imports():
    ...
    try:
        from projects.SeqGrowGraph.seq_grow_graph.lane_diffusion import (
            SwinTransformerUNet,
            LPIM,
            LPDM,
            LanePriorRefinement,
...
def test_swin_unet():
    ...
    try:
        from projects.SeqGrowGraph.seq_grow_graph.lane_diffusion import SwinTransformerUNet
...
def test_lpim():
    ...
    print("\nTesting LPIM...")
    try:
        from projects.SeqGrowGraph.seq_grow_graph.lane_diffusion import LPIM
        import numpy as np
...
def test_lpdm():
    ...
    print("\nTesting LPDM...")
    try:
        from projects.SeqGrowGraph.seq_grow_graph.lane_diffusion import LPDM
        ...
        with torch.no_grad():
            loss_dict = model.compute_loss(x0, xc)
        
        loss_str = ", ".join([f"{k}: {v.item():.4f}" for k, v in loss_dict.items()])
        print(f"✓ LPDM loss computation works. {loss_str}")
        
        # Test sampling
        with torch.no_grad():
            sampled, seg_mask = model.sample(xc, num_steps=5)  # Fewer steps for testing
        
        assert sampled.shape == (B, C, H, W), f"Sampled shape mismatch: {sampled.shape}"
        assert seg_mask.shape[0] == B, "Seg mask batch mismatch"
        print(f"✓ LPDM sampling works. Sampled: {sampled.shape}, SegMask: {seg_mask.shape}")
        return True
    except Exception as e:
...
def test_lane_diffusion():
    ...
    try:
        from projects.SeqGrowGraph.seq_grow_graph.lane_diffusion import LaneDiffusion
        import numpy as np
        ...
        with torch.no_grad():
            out_bev, out_mask = model(raw_bev)
        assert out_bev.shape == (B, C, H, W)
        assert out_mask.shape[0] == B
        print(f"  ✓ Stage III/Inference works. Output: {out_bev.shape}, SegMask: {out_mask.shape}")
